...
func Init() {
	if snRegex, err := regexp.Compile("5G:mnc[0-9]{3}[.]mcc[0-9]{3}[.]3gppnetwork[.]org"); err != nil {
		...
		xAppContext.snRegex = snRegex
	}
	InitxAppContext(&xAppContext)
}
...
func AddAusfUeContextToPool(ausfUeContext *AusfUeContext) {
	xAppContext.UePool.Store(ausfUeContext.Supi, ausfUeContext)
}

func CheckIfAusfUeContextExists(ref string) bool {
	_, ok := xAppContext.UePool.Load(ref)
	return ok
}

func GetAusfUeContext(ref string) *AusfUeContext {
	context, _ := xAppContext.UePool.Load(ref)
...
func AddSuciSupiPairToMap(supiOrSuci string, supi string) {
	...
	newPair.Supi = supi
	xAppContext.suciSupiMap.Store(supiOrSuci, newPair)
}

func CheckIfSuciSupiPairExists(ref string) bool {
	_, ok := xAppContext.suciSupiMap.Load(ref)
	return ok
}

func GetSupiFromSuciSupiMap(ref string) (supi string) {
	val, _ := xAppContext.suciSupiMap.Load(ref)
...
func IsServingNetworkAuthorized(lookup string) bool {
	if xAppContext.snRegex.MatchString(lookup) {
		return true
	} else {
		return false
	}
...
func GetSelf() *XAppContext {
	return &xAppContext
}

func (a *XAppContext) GetSelfID() string {
	return a.NfId
}
